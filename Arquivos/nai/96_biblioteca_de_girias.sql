-- =====================================================================
-- NAI -- 96: a biblioteca de palavras do corretor (Tel, 22/09/2026)
--
-- Ele: "alguns corretores usam uma linguagem informal e coloquial... vamos
-- fazer uma biblioteca de palavras que a Nay vai consultar e deixaremos
-- alguns significados prontos para ela entender o que o corretor quer com
-- aquelas palavras. Ex: '750 mil ta valendo?' -- ele esta perguntando se o
-- imovel esta disponivel".
--
-- ONDE ENTRA: no `contexto` do cabecalho, o mesmo campo que ja diz o
-- tratamento e se a conversa esta em andamento. O fluxo do n8n ja le esse
-- campo (cab.contexto), entao a biblioteca funciona sem mexer no n8n e sem
-- reiniciar nada -- palavra nova vale na mensagem seguinte.
--
-- POR QUE NAO NO PROMPT: gíria e lista que cresce. No prompt, cada palavra
-- nova pesaria em TODA mensagem, inclusive nas que nao tem gíria nenhuma.
-- Aqui so entra a explicacao da palavra que ELE escreveu naquela mensagem.
--
-- COMO ESCREVER A REGRA: `expressao` e uma regex (sem acento, minuscula), e o
-- texto do corretor chega sem acento e em minuscula. Exemplo:
--   expressao: '\mta (valendo|velendo|de pe|em pe)\M'
--   significado: 'ele pergunta se o imovel ainda esta disponivel...'
-- =====================================================================
...
CREATE TABLE IF NOT EXISTS nai_giria (
  id          bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  expressao   text NOT NULL,                 -- regex, minuscula e sem acento
  significado text NOT NULL,                 -- o que a Nay tem que entender
  exemplo     text,                          -- a frase real que originou
  ativo       boolean NOT NULL DEFAULT true,
  criado_por  text,
  criado_em   timestamptz NOT NULL DEFAULT now()
);
CREATE UNIQUE INDEX IF NOT EXISTS nai_giria_expressao ON nai_giria (expressao);

-- A explicacao do que ELE escreveu nesta mensagem. Vazio quando nao ha giria.
CREATE OR REPLACE FUNCTION public.nai_giria_explica(p_texto text)
 RETURNS text
 LANGUAGE sql
 STABLE
AS $function$
  SELECT coalesce(
    ' COMO ELE FALA: ' || string_agg('"' || g.exemplo_curto || '" = ' || g.significado, ' ' ORDER BY g.id),
    '')
    FROM (
      SELECT id, significado,
             -- a expressao inteira que ele escreveu: a regex vai entre
             -- parenteses para o grupo 1 ser o casamento todo, e nao o
             -- primeiro pedaco dela ("ta" em vez de "ta valendo").
             coalesce((regexp_match(lower(unaccent(coalesce(p_texto, ''))), '(' || expressao || ')'))[1],
                      expressao) AS exemplo_curto
        FROM nai_giria
       WHERE ativo
         AND lower(unaccent(coalesce(p_texto, ''))) ~ expressao
       LIMIT 6            -- teto: gíria e ajuda, nao um dicionario no contexto
    ) g;
$function$;

-- As primeiras palavras. A de cima e a que o Tel mandou guardar.
INSERT INTO nai_giria (expressao, significado, exemplo, criado_por) VALUES
  ('\m(ta|esta|tah) ?(valendo|velendo|de pe|em pe)\M',
   'ele pergunta se o imovel ainda esta disponivel, pelo valor que ele citou. Responda a disponibilidade e confirme o valor.',
   'Nay esse edificio Saint Honore 750 mil ta valendo?', 'Tel, 22/09'),
  ('\m(ainda )?(ta|esta) (de pe|na mesa|rolando)\M',
   'mesma coisa que "ta valendo": ele quer saber se o imovel continua disponivel.',
   'ainda ta de pe?', 'Tel, 22/09'),
  ('\m(me )?(joga|passa|manda) (ai|ae|pra mim|pra ca)\M',
   'ele esta pedindo que voce mande o imovel: o card e as fotos.',
   'me joga ai', 'Tel, 22/09'),
  ('\m(fechou|fechado|top|show|suave|beleza|blz|de boa)\M',
   'e concordancia, nao pedido: ele esta dizendo que esta certo. Nao ofereca imovel por causa disso.',
   'fechou', 'Tel, 22/09'),
  ('\m(salve|opa|e ai|eai|fala)\M',
   'e um cumprimento informal, como "oi". Devolva o cumprimento no seu tom.',
   'salve Nay', 'Tel, 22/09'),
  ('\m(quanto (ta|esta|sai|fica)|qual o precinho|ta quanto)\M',
   'ele pergunta o valor do imovel.',
   'quanto ta?', 'Tel, 22/09')
ON CONFLICT (expressao) DO UPDATE
  SET significado = EXCLUDED.significado, exemplo = EXCLUDED.exemplo, ativo = true;

COMMIT;

-- Prova com a frase real do corretor 9284454760 e com frases que NAO sao giria.
SELECT t AS mensagem, nai_giria_explica(t) AS o_que_a_nay_recebe
  FROM unnest(ARRAY[
    'Nay esse edificio saint honoré 750 mil tá valendo?',
    'salve Nay, me joga aí aquele do Aleixo',
    'fechou, obrigado',
    'quanto tá o aluguel do 5727?',
    'tem apartamento de 2 quartos no Parque 10?'
  ]) t;
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    -- A BIBLIOTECA DE PALAVRAS (96, Tel 22/09) entra colada no contexto: so a
    -- explicacao da giria que ELE escreveu nesta mensagem, e nada quando nao
    -- ha giria nenhuma.
    'contexto', nai_contexto(v_papel, v_contato, v_visita, v_teste)
                || CASE WHEN v_papel IN ('corretor', 'tel')
                        THEN nai_giria_explica(v_texto) ELSE '' END,
