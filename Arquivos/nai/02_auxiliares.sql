...
RETURNS text LANGUAGE sql IMMUTABLE AS $$
  SELECT CASE
    ...
    -- Celular que o WhatsApp entrega SEM o 9 (12 digitos, comeca com 6-9):
    -- o numero de verdade tem o 9. Nos avisos ao Tel saia "(96) 9171-2835" (13/09).
    WHEN d ~ '^55[0-9]{2}[6-9][0-9]{7}$' THEN '(' || substr(d,3,2) || ') 9' || substr(d,5,4) || '-' || substr(d,9,4)
...
-- PARAR O CHAT (Tel, 13/09/2026): "todas as vezes que o fluxo para e a regra
-- que ja temos hoje de subir para o tel" -- e a Nay para naquele chat INTEIRO,
-- como na captacao. Motivo 'visita #N: ...' deixa o comando VISITA N do Tel
-- devolver so os chats parados por aquela visita; o resto volta com DEVOLVER.
-- coalesce: chat que ja estava parado por outro motivo continua com o motivo dele.
CREATE OR REPLACE FUNCTION nai_parar_chat(p_contato bigint, p_motivo text)
RETURNS void LANGUAGE sql AS $$
  UPDATE nai_contato SET humano_assumiu_em = coalesce(humano_assumiu_em, now()),
                         humano_motivo = coalesce(humano_motivo, left(p_motivo, 200))
   WHERE id = p_contato
     -- Excecao (Tel, 13/09: "poe a ia para voltar a me responder mesmo que escale
     -- para o tel, mas so para o meu numero"): nai_config.escala_sem_parar.
     AND NOT (chave = ANY (SELECT nai_chave(btrim(x)) FROM unnest(string_to_array(nai_cfg('escala_sem_parar', ''), ',')) x
                            WHERE btrim(x) <> ''));
$$;

CREATE OR REPLACE FUNCTION nai_liberar_chats_da_visita(p_visita bigint)
RETURNS int LANGUAGE sql AS $$
  WITH u AS (UPDATE nai_contato SET humano_assumiu_em = NULL, humano_motivo = NULL
              WHERE humano_motivo LIKE 'visita #' || p_visita || ':%' RETURNING 1)
