-- =====================================================================
-- NAI -- 87: "ALUGADO 5727" pelo WhatsApp do Tel (22/09/2026)
--
-- Ele: "o Tel vai te passar quando um imovel for alugado, tu torna ele
-- indisponivel, prepara o sistema para receber isso, do whatsapp dele".
--
-- COMO FICA, no WhatsApp dele:
--     ALUGADO 5727              -> sai do ar (Nay e site)
--     ALUGADO 5727 e 5733       -> os dois de uma vez
--     VENDIDO 3210              -> mesma coisa, status Vendido
--     INDISPONIVEL 5721         -> fora do ar sem dizer por que
--     DISPONIVEL 5727           -> volta para o ar
-- Aceita tambem ALUGOU/ALUGUEI/VENDEU/VENDI/FECHOU/SAIU e LIBERA/VOLTOU.
--
-- O QUE MUDA NO CADASTRO: `disponivel` e `status`. Escolhido assim porque
--   * a Nay le o catalogo por `nay_esta_no_mercado(disponivel, bloqueado,
--     publicado_no_site)` -- com disponivel=false ela para de oferecer, de
--     mandar card e de responder que tem;
--   * o site lista com `publicado_no_site = true AND disponivel = true`, entao
--     some da vitrine tambem;
--   * a sincronizacao do catalogo NAO sobrescreve `disponivel` (ele esta
--     marcado como nao sincronizado no app.py), entao a marcacao dele nao
--     volta atras sozinha.
-- `publicado_no_site` fica como esta: e a chave de "existe no site", e mexer
-- nela mudaria o que a varredura entende por imovel do site.
--
-- FICA O RASTRO em `imovel_situacao_log`: codigo, o que era, o que virou, quem
-- mandou e quando. Sem isso ninguem sabe por que um imovel sumiu.
-- =====================================================================
...
CREATE TABLE IF NOT EXISTS imovel_situacao_log (
  id            bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  codigo        integer NOT NULL,
  de_disponivel boolean,
  para_disponivel boolean NOT NULL,
  status_antes  text,
  status_depois text,
  quem          text NOT NULL,
  comando       text,
...
CREATE INDEX IF NOT EXISTS imovel_situacao_log_codigo ON imovel_situacao_log (codigo, criado_em DESC);

CREATE OR REPLACE FUNCTION public.nai_marcar_imovel(p_verbo text, p_resto text, p_quem text)
 RETURNS text
 LANGUAGE plpgsql
AS $function$
DECLARE
  v_volta   boolean := lower(unaccent(coalesce(p_verbo, ''))) ~ '^(disponivel|disponibiliza|liber|voltou)';
  v_status  text;
  v_cods    int[];
  c         int;
  i         imoveis;
...
BEGIN
  v_status := CASE
    WHEN v_volta THEN 'Disponível'
    WHEN lower(unaccent(coalesce(p_verbo, ''))) ~ '^(vend)' THEN 'Vendido'
    WHEN lower(unaccent(coalesce(p_verbo, ''))) ~ '^(alug)' THEN 'Alugado'
    ELSE 'Indisponível' END;

  -- os codigos da mensagem: "5727", "5727 e 5733", "#5727, 5733"
  SELECT coalesce(array_agg(DISTINCT (m.x)[1]::int), '{}') INTO v_cods
    FROM regexp_matches(coalesce(p_resto, ''), '(\d{3,5})', 'g') AS m(x);

  IF coalesce(array_length(v_cods, 1), 0) = 0 THEN
    RETURN 'Me diz o código do imóvel: ' || upper(p_verbo) || ' 5727.';
  END IF;

  FOREACH c IN ARRAY v_cods LOOP
    SELECT * INTO i FROM imoveis WHERE codigo = c;
    IF i.codigo IS NULL THEN
      v_nao := v_nao || c::text;
    ELSIF coalesce(i.disponivel, true) = v_volta THEN
      v_ja := v_ja || (c::text || ' (' || coalesce(i.status, CASE WHEN v_volta THEN 'disponível' ELSE 'indisponível' END) || ')');
    ELSE
      INSERT INTO imovel_situacao_log (codigo, de_disponivel, para_disponivel, status_antes, status_depois, quem, comando)
      VALUES (c, i.disponivel, v_volta, i.status, v_status, p_quem, upper(p_verbo) || ' ' || p_resto);
      -- a tabela `imoveis` nao tem carimbo de atualizacao; quem guarda o
      -- quando e o `imovel_situacao_log` logo acima.
      UPDATE imoveis SET disponivel = v_volta, status = v_status WHERE codigo = c;
      v_ok := v_ok || (c::text || ' — ' || coalesce(nai_condominio_ok(i.condominio_nome), coalesce(nullif(i.tipo,''), 'imóvel')) ||
                       coalesce(' (' || nullif(i.bairro, '') || ')', ''));
    END IF;
  END LOOP;

  RETURN btrim(  -- btrim so tira espaco por padrao; a quebra de linha vai junto
    CASE WHEN array_length(v_ok, 1) > 0 THEN
      CASE WHEN v_volta THEN 'Voltou para o ar: ' ELSE 'Tirei do ar (' || lower(v_status) || '): ' END
      || array_to_string(v_ok, '; ')
      || CASE WHEN v_volta THEN '. Já volto a oferecer.' ELSE '. Não ofereço mais nem no site.' END
    ELSE '' END ||
    CASE WHEN array_length(v_ja, 1) > 0 THEN
      E'\n' || 'Já estava assim: ' || array_to_string(v_ja, '; ') || '.' ELSE '' END ||
    CASE WHEN array_length(v_nao, 1) > 0 THEN
      E'\n' || 'Não achei no cadastro: ' || array_to_string(v_nao, ', ') || '.' ELSE '' END, E' 
');
END;
$function$;
...
BEGIN
  ...
  -- CORRETOR <telefone> / NAO CORRETOR <telefone> (Tel, 18/09/2026): a
  -- resposta dele ao "e corretor?" que a NAI manda quando alguem de fora dos
  -- grupos pergunta de imovel. A logica mora em nai_decidir_corretor (36c).
  m := regexp_match(s, '^(n[aã]o\s+)?corretor\s+(.+)$', 'i');
  IF m IS NOT NULL THEN
    v_msg := nai_decidir_corretor(m[2], m[1] IS NULL);
    RETURN QUERY SELECT v_msg; RETURN;
  END IF;
  ...
  -- IMOVEL ALUGADO / VENDIDO (Tel, 22/09/2026): "o Tel vai te passar quando um
  -- imovel for alugado, tu torna ele indisponivel, prepara o sistema para
  -- receber isso, do whatsapp dele". Aceita varios codigos na mesma mensagem
  -- ("ALUGADO 5727 e 5733") e a volta atras ("DISPONIVEL 5727").
  m := regexp_match(s, '^(alugad[oa]|alugou|aluguei|vendid[oa]|vendeu|vendi|indisponivel|indispon[ií]vel|dispon[ií]vel|disponibiliza|liberar?|voltou)\s+(.*)$', 'i');
  IF m IS NOT NULL THEN
    v_msg := nai_marcar_imovel(m[1], m[2], 'Tel pelo WhatsApp');
    PERFORM nai_enfileirar_texto(p_turno, NULL, t.contato_id, 'turno', v_msg, 'comando_tel', 1);
    RETURN QUERY SELECT v_msg; RETURN;
  END IF;
...
-- ---------------------------------------------------------------------
-- A PENEIRA: sem isto a mensagem nao chega no leitor de comandos e vira
-- conversa normal. Exige um codigo de 3 a 5 digitos na mesma linha.
-- "fechou" e "saiu" ficaram FORA de proposito: sao palavras do dia a dia
-- ("fechou 3500" e negociacao, nao imovel alugado).
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.nai_e_comando_imovel(p text)
 RETURNS boolean
 LANGUAGE sql
 IMMUTABLE
AS $function$
  SELECT coalesce(p, '') ~*
    ('^\s*(@?nay[\s,:;.!-]*)?(alugad[oa]|alugou|aluguei|vendid[oa]|vendeu|vendi|'
     || 'indispon[ií]vel|dispon[ií]vel|disponibiliza|liberar?|voltou)\s+[^\n]*\d{3,5}');
$function$;

CREATE OR REPLACE FUNCTION public.nai_e_comando_extra(p text)
 RETURNS boolean
 LANGUAGE sql
 STABLE
AS $function$
  SELECT nai_e_comando_agenda(p) OR nai_e_comando_parada(p) OR nai_e_comando_corretor(p)
      OR nai_e_comando_imovel(p);
$function$;
