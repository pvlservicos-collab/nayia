-- =====================================================================
-- NAI -- 55: o imovel reconhecido pelo nome do condominio (Tel, 16/09/2026)
--
-- O caso: o corretor Luba (Xx Luciano Braz) mandou as 11:56 a ficha de um
-- imovel copiada de outro sistema --
--
--     Apartamento / Apartamento - Nossa Senhora de Fatima Residencial
--     Condominio: Nossa Senhora de Fatima Residencial
--     Bairro: Adrianopolis ... Venda R$ 487.000,00
--
-- -- e perguntou "Disponivel para visita?". A Nay nao respondeu.
--
-- O imovel E NOSSO: codigo 2276, e o valor da ficha dele bate com o da base.
-- Mas o card veio de outro sistema e nao tem a linha "Codigo:", entao
-- `nay_codigos_citados` nao achava nada e a porta tratava como conversa antiga
-- sem assunto de imovel.
--
-- AGORA o nome do condominio basta. Sao 31 condominios entre os imoveis que ela
-- atende, e a regra exige nome de 8 letras ou mais, sem barra -- o que deixa de
-- fora "Casa", "Predio", "Loja/Ponto" e "Sala/Andar", que apareceriam em
-- qualquer frase.
--
-- VENDA CONTINUA CALADA, por decisao dele de agora: "por enquanto calada, vou
-- confirmar aqui com o Tel". Isso sai de graca do desenho -- `nai_oferta_serve`
-- e o mesmo crivo da busca e ja deixa os 1048 imoveis de venda de fora. O 2276
-- do exemplo e de venda, entao ele segue com o Tel; o que muda e o card de
-- LOCACAO sem codigo, que antes tambem passava batido.
--
-- Gerado por `scratchpad/patch55.py` a partir do que estava NO BANCO.
-- =====================================================================
...
CREATE OR REPLACE FUNCTION public.nai_imovel_pelo_condominio(p_texto text, p_telefone text)
 ...
 STABLE
AS $function$
  -- CARD SEM CODIGO (Tel, 16/09). O corretor Luba mandou a ficha de um imovel
  -- copiada de outro sistema -- "Apartamento / Nossa Senhora de Fatima
  -- Residencial / Adrianopolis / 115 m2..." -- e perguntou se estava disponivel
  -- para visita. O imovel e nosso, mas a ficha nao trazia a linha "Codigo:", e
  -- por isso nada no sistema o reconhecia.
  --
  -- SO IMOVEL QUE ELA ATENDE: `nai_oferta_serve` e o mesmo crivo da busca, e ele
  -- deixa de fora os de venda. Isso mantem a venda calada, como o Tel pediu
  -- enquanto confere com o Tel se ela deve entrar nesse assunto.
  --
  -- NOME CURTO NAO CONTA: "Casa" e "Predio" sao nomes de condominio na base e
  -- apareceriam em qualquer frase. Daqui para baixo de 8 letras nao vale, e
  -- nome com barra ("Loja/Ponto", "Sala/Andar") e tipo, nao nome.
