...
BEGIN
  ...
  IF v.acesso IS NULL THEN
    v_texto := 'Obrigada! E como fica o acesso ao imóvel? A chave está com a gente, o Fernando pega a chave antes, é fechadura eletrônica, ou prefere estar lá para abrir?';
  ...
  ELSE
    ...
    v_texto := CASE WHEN v.acesso = 'chave_conosco'
      ...
      nai_frase_visitante(v) || ' Qualquer novidade eu aviso por aqui.';
    v_moto := 'Oi, Fernando! Reservamos uma visita no ' || nai_endereco_interno(v.codigo) ||
      coalesce(', do proprietário ' || v.proprietario_nome, '') || ', ' || nai_hora_exata(v.quando) ||
      ', com o corretor ' || coalesce(c.nome_completo, c.nome_whatsapp, '') || ' (' || nai_fone_fmt(c.telefone) || ').' ||
      coalesce(E'\nVisitante: ' || v.visitante_nome || coalesce(', CPF ' || nai_cpf_fmt(v.visitante_cpf), ''), '') ||
      E'\n' || nai_linha_acesso_motoboy(v) ||
      E'\nPosso confirmar que você acompanha?';
...
BEGIN
  ...
  IF t.visita_id IS NULL THEN
    ...
    PERFORM nai_avisar_tel(p_turno, NULL, 'Tel, o proprietário ' || coalesce(k.nome_whatsapp, '') || ' (' ||
      nai_fone_fmt(k.telefone) || ') escreveu e não sei de qual visita é: "' || left(coalesce(t.texto, ''), 300) || '"', 'proprietario_sem_visita');
  ...
  IF dec IN ('outro_horario', 'recusa', 'nao') THEN
    ...
    RETURN QUERY SELECT ('Combinado! Vou ver com o corretor se ' || nai_quando_humano(q) || ' fica bom e já te retorno.')::text, NULL::text;
    RETURN;
  END IF;
  ...
  PERFORM nai_avisar_tel(p_turno, v.id, 'Tel, o proprietário do ' || nai_ref_imovel(v.codigo) || ' (visita #' || v.id ||
    ') perguntou: "' || left(coalesce(nullif(p_detalhe, ''), t.texto, ''), 300) || '"', 'proprietario_duvida');
  RETURN QUERY SELECT NULL::text,
...
DECLARE v nai_visita; c nai_contato; m record; v_nome text;
BEGIN
  SELECT * INTO v FROM nai_visita WHERE id = p_visita FOR UPDATE;
  SELECT * INTO c FROM nai_contato WHERE id = v.corretor_id;
...
BEGIN
  ...
  IF dec IN ('nao_pode', 'nao', 'problema', 'imprevisto') THEN
    ...
    PERFORM nai_avisar_tel(p_turno, v.id, 'Tel, o Fernando disse que NÃO consegue acompanhar a visita #' || v.id || ' (' ||
      ...
      E'.\nNão existe acompanhante reserva. Me responda VISITA ' || v.id || ' HORARIO <novo> ou VISITA ' || v.id || ' CANCELA.',
      'motoboy_nao_pode');
    -- CALADA (Tel, 12/09): foi para o Tel, o corretor nao e avisado.
    RETURN QUERY SELECT NULL::text,
      'ja avisei o Tel. A pessoa NAO fica sabendo (regra do Tel, 12/09): responda exatamente SILENCIO.'::text; RETURN;
  ...
  PERFORM nai_avisar_tel(p_turno, v.id, 'Tel, o Fernando escreveu sobre a visita #' || v.id || ': "' ||
    left(coalesce(nullif(p_obs, ''), t.texto, ''), 300) || '"', 'motoboy_outro');
  RETURN QUERY SELECT NULL::text,
      'ja avisei o Tel. A pessoa NAO fica sabendo (regra do Tel, 12/09): responda exatamente SILENCIO.'::text;
END;
$$;
