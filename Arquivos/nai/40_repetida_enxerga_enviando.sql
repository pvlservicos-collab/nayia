-- =====================================================================
-- NAI -- 40: a trava de repetida tem que enxergar o que esta SAINDO (19/09)
--
-- O BUG, como o Tel viu: "ela tambem esta mandando so as fotos" -- e pior, a
-- Sra. Solange recebeu as MESMAS 7 fotos DUAS VEZES, 16 segundos uma da outra
-- (nai_saida 30022-30028 as 09:51:13 e 30030-30036 as 09:51:29). Nenhuma saiu
-- bloqueada: as 14 foram entregues.
--
-- POR QUE A TRAVA NAO PEGOU: ela procurava mensagem igual em estado 'enviado'
-- ou 'simulado'. Quando a segunda leva foi LIBERADA, a primeira ainda estava
-- em 'enviando' -- liberada, indo para a Z-API, ainda sem confirmacao. Estado
-- que a trava nao olhava. E `enviado_em` ainda era NULL, entao mesmo que
-- olhasse, a comparacao com NULL daria falso.
--
-- A causa de origem (a mesma entrada processada duas vezes) e do fluxo do
-- n8n -- ver [[juntar-mensagens-roda-duas-vezes]]. Esta trava e a ultima
-- parede antes do WhatsApp, e ela tinha um buraco de poucos segundos que era
-- exatamente o tamanho do problema.
-- =====================================================================
