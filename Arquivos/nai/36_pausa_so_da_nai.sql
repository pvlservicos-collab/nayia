-- =====================================================================
-- NAI -- 36: a pausa da Nay ANTIGA nao cala mais a NAI (Tel, 15/09/2026)
--
-- Ele: "a nay parceria que e locacao esta ligada? sinto que pode estar
-- deixando de atender, da uma revisada". Estava ligada e muda.
--
-- O QUE ACONTECEU. `config.atendimento_pausado = 'sim'` e a pausa da Nay
-- ANTIGA, ligada em agosto e de proposito. O cabecalho do turno da NAI lia
-- essa chave tambem, isentando so o numero de teste. Enquanto a NAI era so
-- teste isso nunca apareceu: o unico que falava com ela era o Pedro, e ele e
-- "teste". As 14:12, ao abrir para todos, todo mundo passou a cair naquela
-- pausa -- 133 mensagens entraram, 90 turnos foram abertos e NENHUMA resposta
-- saiu. Sem erro em log nenhum: o roteador simplesmente devolve silencio
-- quando o cabecalho diz pausado.
--
-- Gerado por script a partir do que estava NO BANCO: uma linha trocada.
-- =====================================================================
...
BEGIN
  ...
  SELECT * INTO v_porta FROM nai_deve_atender(p_tel, v_texto, NULL);
