-- =====================================================================
-- NAI -- 19b: o encaixe do AGENDA DE VISITAS nas duas funcoes vivas.
--
-- Gerado por `scratchpad/patch_agenda.py` a partir do que estava NO BANCO,
-- com dois acrescimos e nenhuma remocao:
--   1. `nai_comando_tel` ganha o ramo da agenda ANTES do ramo do VISITAS,
--      que continua identico.
--   2. `nai_abrir_turno` passa a reconhecer o comando vindo dos telefones de
--      `comando_telefones`, e passa a dizer no cabecalho (`cab.comando_nai`)
--      se o texto e comando da NAI -- campo novo, ninguem que ja existia le.
--
-- Rode `19_agenda_de_visitas.sql` ANTES deste arquivo.
-- =====================================================================
...
BEGIN
  ...
  IF v_teste THEN
    ...
    IF v_papel = 'corretor' AND nai_e_comando_agenda(v_texto) AND nai_pode_comandar(v_chave) THEN
      v_papel := 'tel';
    END IF;
  ...
  ELSE
    IF v_chave = nai_chave(nai_cfg('tel_telefone'))
       OR (nai_e_comando_agenda(v_texto) AND nai_pode_comandar(v_chave)) THEN
      v_papel := 'tel';
  ...
  RETURN jsonb_build_object(
    ...
    'comando_nai', (v_papel = 'tel' AND (nai_e_comando_tel(v_texto) OR nai_e_comando_agenda(v_texto))),
